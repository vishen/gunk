...
type builder struct {
	...
	// they are found.
	translatedDeclarations []*strings.Builder

	// If an error is found during processing store the error
	// and error position here.
	err    error
	errPos scanner.Position
...
func Convert(path string, overwrite bool) error {
	...
	for _, e := range d.Elements {
		...
		if b.err != nil {
			return fmt.Errorf("error in %d:%d: %v\n", b.errPos.Line, b.errPos.Column, b.err)
		}
	}
	...
	w := &strings.Builder{}
	w.WriteString(translatedPkg.String())
	w.WriteString("\n\n")
	w.WriteString(translatedImports.String())
	w.WriteString("\n")
	...
	if _, err := os.Stat(fullpath); !os.IsNotExist(err) && !overwrite {
		return fmt.Errorf("path already exists %q, cannot overwrite", fullpath)
	}
...
// Write will write output to a string builder, adding in indentation
// where required. It will write the comment first if there is one,
// and then write the rest.
func (b *builder) Write(w *strings.Builder, indent int, comments *proto.Comment, s string, args ...interface{}) {
	if comments != nil {
...
// emitError will save the current error position and error. The error
// formatting will use these when reporting errors.
func (b *builder) emitError(pos scanner.Position, s string, args ...interface{}) {
	b.errPos = pos
	b.err = fmt.Errorf(s, args...)
}
...
func (b *builder) handleProtoType(typ proto.Visitee) error {
	...
	case *proto.Message:
		b.handleMessage(typ.(*proto.Message))
	case *proto.Enum:
		b.handleEnum(typ.(*proto.Enum))
	case *proto.Service:
		b.handleService(typ.(*proto.Service))
	case *proto.Option:
		o := typ.(*proto.Option)
		lit := o.Constant
		value, err := b.handleLiteralString(lit)
		if err != nil {
			b.emitError(lit.Position, "expected string literal in top level option: %v", err)
			return nil
		}
...
func (b *builder) handleMessageField(w *strings.Builder, field proto.Visitee) error {
	...
	// each language???
	b.Write(w, 1, comment, "%s %s `pb:\"%d\" json:\"%s\"`\n", snaker.ForceCamelIdentifier(name), typ, sequence, snaker.CamelToSnake(name))
	return nil
}
...
func (b *builder) handleMessage(m *proto.Message) {
	w := &strings.Builder{}
	b.Write(w, 0, m.Comment, "type %s struct {\n", m.Name)
	for _, e := range m.Elements {
		switch e.(type) {
		case *proto.NormalField:
			f := e.(*proto.NormalField)
			if err := b.handleMessageField(w, f); err != nil {
				b.emitError(f.Position, "error with message field: %v", err)
				return
			}
		case *proto.Enum:
		...
		case *proto.Comment:
			b.Write(w, 1, e.(*proto.Comment), "")
		case *proto.MapField:
			mf := e.(*proto.MapField)
			if err := b.handleMessageField(w, mf); err != nil {
				b.emitError(mf.Position, "error with message field: %v", err)
				return
			}
		default:
			b.emitError(m.Position, "unexpected type %T in message", e)
			return
		}
	}
	b.Write(w, 0, nil, "}")
	b.translatedDeclarations = append(b.translatedDeclarations, w)
}

func (b *builder) handleEnum(e *proto.Enum) {
	...
	// and increments by 1.
	w := &strings.Builder{}
	b.Write(w, 0, e.Comment, "type %s int\n", e.Name)
	b.Write(w, 0, nil, "\nconst (\n")
	for _, c := range e.Elements {
		ef, ok := c.(*proto.EnumField)
		if !ok {
			b.emitError(e.Position, "unexpected type %T in enum, expected enum field", c)
			return
		}
		b.Write(w, 1, ef.Comment, "%s %s = %d\n", ef.Name, e.Name, ef.Integer)
	}
	b.Write(w, 0, nil, ")")
	b.translatedDeclarations = append(b.translatedDeclarations, w)
}
...
func (b *builder) handleService(s *proto.Service) {
	w := &strings.Builder{}
	b.Write(w, 0, s.Comment, "type %s interface {\n", s.Name)
	for i, e := range s.Elements {
		r, ok := e.(*proto.RPC)
		if !ok {
			b.emitError(s.Position, "unexpected type %T in service, expected rpc", e)
			return
		}
		...
		for _, o := range r.Elements {
			...
			if !ok {
				b.emitError(r.Position, "unexpected type %T in service rpc, expected option", o)
				return
			}
			switch n := opt.Name; n {
			case "(google.api.http)":
				...
				if len(literal.OrderedMap) == 0 {
					b.emitError(opt.Position, "expected option to be a map")
					return
				}
				for _, l := range literal.OrderedMap {
					...
					case "body":
						...
						if err != nil {
							b.emitError(opt.Position, "option for body should be a string")
							return
						}
					default:
						method = n
						...
						if err != nil {
							b.emitError(opt.Position, "option for %q should be a string (url)", method)
							return
						}
					}
				}
				...
				if method != "" && url != "" {
					...
					b.Write(w, 1, nil, "// +gunk http.Match{\n")
					b.Write(w, 1, nil, "//     Method: %q,\n", strings.ToUpper(method))
					b.Write(w, 1, nil, "//     Path: %q,\n", url)
					if body != "" {
						b.Write(w, 1, nil, "//     Body: %q,\n", body)
					}
					b.Write(w, 1, nil, "// }\n")
				}
			default:
				...
				// for now?
				b.emitError(r.Position, "%s option is not yet handled", n)
				return
			}
...
func (b *builder) handlePackage() *strings.Builder {
	w := &strings.Builder{}
	var opt pkgOpt
	...
	// options.
	p := b.pkg
	b.Write(w, 0, p.Comment, "")
	b.Write(w, 0, opt.comment, "")
	b.Write(w, 0, nil, "package %s", p.Name)
	if opt.value != "" {
		b.Write(w, 0, nil, " // proto %s", opt.value)
	}

	return w
}

func (b *builder) handleImports() *strings.Builder {
	w := &strings.Builder{}
	b.Write(w, 0, nil, `import (
	"github.com/gunk/opt"
